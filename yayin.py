#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Canli yayin cekici - GitHub Actions icin.

- Domain numarasi artar (655, 656, ...), en guncel domaini bulur.
- Ana sayfadaki TUM kanal ve mac yayinlarinin direkt m3u8 linklerini ceker.
- Her kanal icin eksiksiz header bilgisi (referrer + user-agent + origin) ekler.
- Ciktilar:
    yayinlar.m3u            -> toplu liste (tum yayinlar)
    kanallar/<isim>.m3u8    -> her kanal icin ayri dosya

Not: placeholder (yayin yok) kanallar da listelenir — URL kalici, yayin
baslayinca ayni link otomatik calisir. Sadece ERISILEMEYEN (HTTP 200 degil)
kanallar elenir.
"""
import requests
import re
...
def api_stream(video_id):
    """streamsport365.com/cinema API'sinden token'li gercek video URL'si alir.

    Oynatici JS'i bu API'yi ID > 10000 kosuluyla kullaniyor; ama API elle
    her ID icin URL donduruyor. e-aga-m placeholder donerse yedek olarak
    denenir. Donen URL token'li (s=, t=) — workflow 30 dk'da tazeler.
    """
    try:
        r = requests.post(
...
def reachable(url):
    """Link erisilebilir mi? (HTTP 200 + m3u8 icerik — placeholder da kabul)"""
    try:
...
def main():
    ...
    msg(f"[i] {len(links)} yayin bulundu, linkler aliniyor...")

    streams = []
    for i, (title, url) in enumerate(links, 1):
        # 1) sayfa ID -> e-aga-m sablonu (token'siz, kalici)
        stream = page_to_stream(url)
        # 2) erisilemezse -> streamsport365 API (token'li)
        if not (stream and reachable(stream)):
            mid = re.search(r"match-center\.php\?id=(\d+)",
                            requests.get(url, headers=HEADERS, timeout=TIMEOUT).text)
            api_url = api_stream(mid.group(1)) if mid else None
            if api_url and reachable(api_url):
                stream = api_url
        if stream and reachable(stream):
            streams.append((title, stream))
            msg(f"  [{i}/{len(links)}] + {title[:45]}")
        else:
            msg(f"  [{i}/{len(links)}] - {title[:45]} (erisilemedi)")
        time.sleep(0.15)
